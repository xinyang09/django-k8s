...
def memoryUnitToG(str):
    if str.endswith("M") or  str.endswith("Mi"):
        ...
        return float(g)
#用于获取Kubernetes集群中所有节点的详细信息，包括节点名称、IP、操作系统、内核版本、可分配资源等。
...
#用于统计每个节点的资源分配情况，包括可分配的CPU和内存，总容量，已请求的资源等。
def node_resource(core_api, n_name=None):
    ...
    for node in core_api.list_node().items:
        ...
        node_resouces[node_name]["allocatable_cpu"] = int(allocatable_cpu.rstrip('m'))
        node_resouces[node_name]["allocatable_memory"] = memoryUnitToG(allocatable_memory)
        node_resouces[node_name]["allocatable_ephemeral_storage"] = round(int(allocatable_storage.rstrip('Ki')) / 1024 / 1024 / 1024,2)
